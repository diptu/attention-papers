...
% Note. For the workshop paper template, both \title{} and \workshoptitle{} are required, with the former indicating the paper title shown in the title and the latter indicating the workshop title displayed in the footnote. 
\title{From Transformers to KV Cache Compression}
...
\begin{abstract}
Attention Papers is a curated, thematic repository documenting the evolution of attention mechanisms—from foundational sequence-to-sequence models to modern, long-context Transformers.

Instead of a static list, it maps the field by the core principles driving innovation: memory efficiency, mathematical approximation, and computational scalability.

Key Features:

Structured Analysis: Each paper includes technical summaries, mathematical intuition, complexity analysis, and historical context.

Thematic Threading: Connects seemingly disparate breakthroughs to show the lineage of ideas like sparse attention and low-rank approximation.

Inference Focus: A dedicated track on KV cache compression, addressing the critical bottleneck of scaling inference for long-context models.
\end{abstract}
